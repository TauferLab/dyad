...
using key_type = std::pair<const ucp_address_t*, const size_t>;
using cache_type = std::map<key_type, ucp_ep_h>;
                                  ...
                                  ucp_ep_h* ep)
{
    ...
    try {
        ...
        auto key = std::make_pair<const ucp_address_t*, const size_t> (std::move(addr), std::move(addr_size));
...
namespace std {
    ...
    struct hash<key_type>
    {
        ...
        result_t operator()(const arg_t& a) const
        {
            ...
            const uint8_t* const byte_buf = reinterpret_cast<const uint8_t* const>(a.first);
