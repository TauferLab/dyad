        ...
        dyad_dtl_t **dtl_handle)
{
    *dtl_handle = malloc(sizeof(struct dyad_dtl));
    ...
    (*dtl_handle)->h = h;
    if (mode == DYAD_DTL_HG) {
        return dyad_dtl_mercury_init (h, kvs_namespace, debug,
                (dyad_dtl_mercury_t**)&(*dtl_handle)->real_dtl_handle);
    }
        ...
        json_t **packed_obj)
{
    if (dtl_handle->mode == DYAD_DTL_HG) {
        return dyad_dtl_mercury_rpc_pack(
            (dyad_dtl_mercury_t*) dtl_handle->real_dtl_handle,
            upath,
            producer_rank,
            packed_obj
...
dyad_rc_t dyad_dtl_recv_rpc_response(dyad_dtl_t* dtl_handle, flux_future_t *f)
{
    if (dtl_handle->mode == DYAD_DTL_HG)
    {
        return dyad_dtl_mercury_recv_rpc_response(
            (dyad_dtl_mercury_t*) dtl_handle->real_dtl_handle,
            f
        );
    }
...
dyad_rc_t dyad_dtl_establish_connection(dyad_dtl_t *dtl_handle) {
    if (dtl_handle->mode == DYAD_DTL_HG) {
        return dyad_dtl_mercury_establish_connection(
            (dyad_dtl_mercury_t*) dtl_handle->real_dtl_handle
        );
    }
    if (dtl_handle->mode == DYAD_DTL_FLUX_RPC) {
...
{
    if (dtl_handle->mode == DYAD_DTL_HG) {
        return dyad_dtl_mercury_close_connection(
            (dyad_dtl_mercury_t*) dtl_handle->real_dtl_handle
        );
    }
    if (dtl_handle->mode == DYAD_DTL_FLUX_RPC) {
...
dyad_rc_t dyad_dtl_finalize(dyad_dtl_t **dtl_handle)
{
    ...
    free(*dtl_handle);
    if (mode == DYAD_DTL_HG) {
        return dyad_dtl_mercury_finalize(
            (dyad_dtl_mercury_t*) real_dtl_handle
        );
    }
    if (mode == DYAD_DTL_FLUX_RPC) {
